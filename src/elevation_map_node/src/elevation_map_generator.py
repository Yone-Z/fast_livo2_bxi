...
from height_map_plot import HeightMapPlotter
from matplotlib.animation import FuncAnimation
import matplotlib.pyplot as plt

class GlobalGridCell:
    "全局地图每个网格上的点云历史,提供估计值"
    ...
    def add_point(self, new_elevation: float, time: Time):
        ...
        if not self.has_data:
            ...
            self.avg_elevation = (self.avg_elevation * self.point_count + new_elevation) / (self.point_count + 1) # 蒙特卡洛估计
            self.point_count += 1
...
class ElevationMapGenerator(Node):
    ...
    def __init__(self):
        ...
        # 采样的高程图定义
        self.declare_parameter('local_grid_size', 0.1) # 10cm
        ...
        self.local_grid_width = len(x_points)
        self.local_grid_height = len(y_points)

        # 全局地图定义
        self.declare_parameter('global_grid_size', 0.02) # 2cm
        self.declare_parameter('global_map_width', 10.0)
        self.declare_parameter('global_map_height', 10.0)
        ...
        self.declare_parameter('data_timeout_sec', 300.0) # 超过这个时间的数据将被删除
        ...
        self.cloud_sub = self.create_subscription(
            PointCloud2,
            '/cloud_registered', # Fast livox2发布的过滤后的点云(GCS)
            self.cloud_callback,
            10
        )
        ...
        self.odom_sub = self.create_subscription(
            Odometry,
            '/aft_mapped_to_init', # Fast livox2发布的过滤后的odom
            self.odom_callback,
            10
        )
        ...
        self.global_height_plotter = HeightMapPlotter(self.global_grid_width, self.global_grid_height, self.global_grid_size, 1.0/20)
        self.local_height_plotter = HeightMapPlotter(self.local_grid_width, self.local_grid_height, self.local_grid_size, 0.1/20)

        # self.ani1 = FuncAnimation(fig=self.global_height_plotter.fig,
        #                         func=self.global_height_plotter.update_plot,
        #                         frames=360,
        #                         interval=1000/self.global_height_plotter.refresh_freq,
        #                         blit=False)
        # self.ani2 = FuncAnimation(fig=self.local_height_plotter.fig,
        #                         func=self.local_height_plotter.update_plot,
        #                         frames=360,
        #                         interval=1000/self.local_height_plotter.refresh_freq,
        #                         blit=False)
        # plt.show(False)
    ...
    def parse_pointcloud2(self, msg: PointCloud2):
        ...
        point_step = msg.point_step # 每个点数据长度

        data = msg.data
        # 解析每个点
        for i in range(0, len(data), point_step):
            if i + point_step > len(data): break
    ...
    def get_local_elevation_map(self):
        ...
        elevation_map = np.zeros_like(self.local_grid[0], dtype=float)
        point_num = np.zeros_like(self.local_grid[0], dtype=float)
        ...
        for (grid_x, grid_y), cell in self.global_elevation_map.items():
            if cell.is_valid() and cell.point_count >= self.min_points_per_cell:
                ...
                x_gcs, y_gcs = self.global_index_to_coordinate(grid_x, grid_y)
                ...
                # 转换到局部网格坐标
                grid_x, grid_y = self.local_coordinate_to_index(local_x, local_y)
                
                if (grid_x < 0 or grid_x >= self.local_grid_height or 
                    grid_y < 0 or grid_y >= self.local_grid_width):
                    return
                
                elevation_map[grid_x, grid_y] += cell.avg_elevation
                point_num[grid_x, grid_y] += 1
        
        point_num[point_num == 0] = 1 # 防止除0
        self.local_map_data = elevation_map / point_num
        self.local_height_plotter.height_map=self.local_map_data
        print("local",self.local_map_data.shape)
        ...
        for (grid_x, grid_y), cell in self.global_elevation_map.items():
            if cell.is_valid() and cell.point_count >= self.min_points_per_cell:
                ...
                if (0 <= grid_x < self.global_grid_width and 
                    ...
                    global_map_data[index] = cell.avg_elevation
        
        # 填充地图数据
        self.normalize_and_fill_grid(global_map_data, grid_msg)

        self.global_map_data = global_map_data.reshape(self.global_grid_width, self.global_grid_height)
        self.global_height_plotter.height_map=self.global_map_data
        print("global:",self.global_map_data.shape)

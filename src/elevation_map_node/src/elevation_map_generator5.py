...
# 可视化模块
from height_map_plot2 import HeightMapPlotter  # 绘图文件
...
class GlobalGridCell:
    ...
    def __init__(self):
        ...
        # 楼梯检测相关
        self.elevation_history = []           # 保存最近的高程值用于楼梯检测
        self.history_limit = 10               # 历史数据保存数量限制
        self.dominant_elevation = 0.0         # 主导高程值（楼梯台阶的代表高度）
        self.is_stair_cell = False           # 标记是否为楼梯网格
    ...
    def add_point(self, new_elevation: float, time: Time):
        ...
        if len(self.elevation_history) > self.history_limit:
            self.elevation_history.pop(0)  # 移除最老的数据
        
        if not self.has_data:
            # 首次添加点：直接初始化
            self.max_elevation = new_elevation
            self.avg_elevation = new_elevation
            self.dominant_elevation = new_elevation
            self.point_count = 1
            self.has_data = True
        else:
            # 增量更新：
            ...
            # 4. 楼梯检测和主导高程计算
            self._update_stair_detection()
        
        # 更新时间戳
        self.last_update = time
    
    def _update_stair_detection(self):
        """
        更新楼梯检测和主导高程计算
        """
        if len(self.elevation_history) < 3:
            return
        ...
        for elevation in elevations:
            ...
            for level in unique_levels:
                if abs(elevation - level) < step_threshold:
                    found_level = True
                    break
            if not found_level:
                unique_levels.append(elevation)
        
        # 如果有多个明显的高度级别，认为是楼梯
        if len(unique_levels) >= 2:
            self.is_stair_cell = True
            # 使用最常出现的高度作为主导高程（众数）
            self.dominant_elevation = self._calculate_mode_elevation(elevations, step_threshold)
        else:
            self.is_stair_cell = False
            self.dominant_elevation = self.avg_elevation
    
    def _calculate_mode_elevation(self, elevations, threshold):
        """
        计算高程值的众数（最常出现的高度级别）
        
        Args:
            elevations (np.ndarray): 高程值数组
            threshold (float): 分组阈值
        ...
        for elevation in elevations:
            ...
            for group in groups:
                if any(abs(elevation - g) < threshold for g in group):
                    group.append(elevation)
                    added_to_group = True
                    break
            if not added_to_group:
                groups.append([elevation])
        
        # 找到最大的组（最常出现的高度级别）
        if not groups:
            return elevations[-1]  # 返回最新值
        
        largest_group = max(groups, key=len)
        return np.mean(largest_group)  # 返回该组的平均值
    
    def get_representative_elevation(self, prefer_stairs=True):
        """
        获取代表性高程值
        
        Args:
            prefer_stairs (bool): 是否优先使用楼梯模式的主导高程
            
        Returns:
            float: 代表性高程值
        """
        if prefer_stairs and self.is_stair_cell:
            return self.dominant_elevation
        else:
...
class ElevationMapGenerator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('global_grid_size', 0.05)   # 全局地图网格大小：cm
        ...
        self.stair_detection_enabled = self.get_parameter('stair_detection_enabled').get_parameter_value().bool_value
        self.stair_step_threshold = self.get_parameter('stair_step_threshold').get_parameter_value().double_value
        ...
        if self.stair_detection_enabled:
            self.global_height_plotter.set_stair_mode(True, self.stair_step_threshold, True)
            self.local_height_plotter.set_stair_mode(True, self.stair_step_threshold, True)
            self.get_logger().info(f'Stair mode enabled with threshold: {self.stair_step_threshold}m')
        else:
            self.global_height_plotter.set_stair_mode(False)
            self.local_height_plotter.set_stair_mode(False)
            self.get_logger().info('Stair mode disabled - using smooth surface')
    ...
    def get_local_elevation_map(self):
        ...
        for (global_grid_x, global_grid_y), cell in self.global_elevation_map.items():
            ...
            if cell.is_valid() and cell.point_count >= self.min_points_per_cell:
                ...
                # 根据楼梯检测结果选择合适的高程值
                if self.stair_detection_enabled:
                    # 楼梯模式：使用代表性高程（主导高程或平均值）
                    representative_elevation = cell.get_representative_elevation(prefer_stairs=True)
                else:
                    # 普通模式：使用平均高程
                    representative_elevation = cell.avg_elevation
                
                # 只有在已经找到最低点时才进行相对高程计算
                if self.global_lowest_elevation != float('inf'):
                    relative_elevation = representative_elevation - self.global_lowest_elevation
                    # 调试：记录一些样本的相对高程计算
                    if valid_local_points < 3:  # 只记录前几个点
                        stair_info = f"(stair:{cell.is_stair_cell})" if self.stair_detection_enabled else ""
                        print(f"DEBUG elevation calc: cell.repr={representative_elevation:.6f}, lowest={self.global_lowest_elevation:.6f}, relative={relative_elevation:.6f} {stair_info}")
                else:
                    relative_elevation = representative_elevation  # 还没找到最低点时使用绝对高程
                    if valid_local_points < 3:  # 只记录前几个点
                        stair_info = f"(stair:{cell.is_stair_cell})" if self.stair_detection_enabled else ""
                        print(f"DEBUG elevation calc (no lowest): cell.repr={representative_elevation:.6f}, relative={relative_elevation:.6f} {stair_info}")

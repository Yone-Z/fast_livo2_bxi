...
class ElevationMapGenerator(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('global_grid_size', 0.02)
        self.declare_parameter('global_map_width', 50.0)
        self.declare_parameter('global_map_height', 50.0)
    ...
    def cloud_callback(self, msg: PointCloud2):
        ...
        # 重置局部高程图
        self.local_elevation_map.fill(float('-inf'))
        ...
        for point in points:
            ...
            # 更新局部高程图（使用机器人坐标系）
            self.update_local_map(x, y, z, robot_x, robot_y, yaw)
            valid_points += 1
    ...
    def update_local_map(self, x: float, y: float, z: float, 
        ...
        # 更新局部最大高程值
        if (self.local_elevation_map[grid_x, grid_y] == float('-inf') or 
            z > self.local_elevation_map[grid_x, grid_y]):

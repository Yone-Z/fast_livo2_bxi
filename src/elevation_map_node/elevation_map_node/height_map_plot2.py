...
class HeightMapPlotter:
    def __init__(self, len_x, len_y, dx, dt, title="Height Map"):
        ...
        self.step_threshold = 0.05  # 台阶高度阈值（5cm），低于此值认为是同一层
        self.use_stepped_surface = True  # 使用阶梯式表面而不是平滑插值
        self.voxel_mode = False  # 可选：体素化显示模式
    ...
    def update_plot(self):
        ...
        # 楼梯模式处理
        if self.stair_mode:
            plot_data = self.discretize_heights_for_stairs(plot_data)
            print(f"Applied stair discretization with threshold {self.step_threshold}m")
        
        # 选择绘制方法
        if self.stair_mode and self.use_stepped_surface:
            # 阶梯式表面绘制（无插值）
            self.surf = self.ax.plot_surface(self.X, self.Y, plot_data, 
                                            ...
                                            linewidth=0.5,
                                            antialiased=False,
                                            rstride=1, cstride=1,  # 每个网格都绘制，不跳过
                                            shade=False)  # 关闭光影效果，使台阶更清晰
        elif self.voxel_mode:
            # 体素化显示（每个非零网格显示为立方体）
            self.draw_voxels(plot_data)
        else:
            # 传统平滑表面绘制
            self.surf = self.ax.plot_surface(self.X, self.Y, plot_data, 
                                            cmap='terrain', 
    ...
    def set_stair_mode(self, enabled=True, step_threshold=0.05, use_stepped_surface=True):
        """
        设置楼梯显示模式
        
        Args:
            enabled (bool): 是否启用楼梯模式
            step_threshold (float): 台阶高度阈值，单位米
            use_stepped_surface (bool): 是否使用阶梯式表面
        """
        self.stair_mode = enabled
        self.step_threshold = step_threshold
        self.use_stepped_surface = use_stepped_surface

    def discretize_heights_for_stairs(self, height_map):
        """
        对高程数据进行离散化处理，使其更适合楼梯显示
        
        Args:
            height_map (np.ndarray): 原始高程数据
            
        Returns:
            np.ndarray: 离散化后的高程数据
        """
        if not self.stair_mode:
            return height_map
        
        # 只处理有效数据（非零且有限的值）
        valid_mask = np.isfinite(height_map) & (height_map != 0)
        if not np.any(valid_mask):
            return height_map
        
        discretized = height_map.copy()
        ...
        # 生成离散的台阶高度级别
        step_levels = np.arange(min_height, max_height + self.step_threshold, self.step_threshold)
        ...
        for i in range(height_map.shape[0]):
            for j in range(height_map.shape[1]):
                if valid_mask[i, j]:
                    ...
                    # 找到最近的台阶级别
                    closest_level = step_levels[np.argmin(np.abs(step_levels - current_height))]
                    discretized[i, j] = closest_level
        
        return discretized

    def draw_voxels(self, height_map):
        """
        体素化显示方法 - 将每个有效网格显示为立方体
        
        Args:
            height_map (np.ndarray): 高程数据
        """
        # 获取有效数据的位置
        valid_mask = np.isfinite(height_map) & (height_map != 0)
        ...
        # 为体素显示创建3D网格
        voxel_size = min(self.dx, self.dx)  # 体素大小
        ...
        for i in range(height_map.shape[0]):
            for j in range(height_map.shape[1]):
                if valid_mask[i, j]:
                    ...
                    x_center = self.X[i, j]
                    y_center = self.Y[i, j]
                    z_height = height_map[i, j]
                    
                    # 创建立方体的8个顶点
                    half_size = voxel_size / 2
                    
                    # 底面四个顶点（z=0）
                    x_coords = [x_center - half_size, x_center + half_size, 
                               x_center + half_size, x_center - half_size]
                    y_coords = [y_center - half_size, y_center - half_size, 
                    ...
                    bottom_z = [0, 0, 0, 0]
                    # 顶面
                    top_z = [z_height, z_height, z_height, z_height]
                    
                    # 绘制底面和顶面
                    self.ax.plot_trisurf(x_coords + x_coords[0:1], 
                                        y_coords + y_coords[0:1], 
                                        bottom_z + bottom_z[0:1], 
                                        alpha=0.7, color='brown')
                    
                    if z_height > 0.01:  # 只有足够高度才绘制顶面
                        self.ax.plot_trisurf(x_coords + x_coords[0:1], 
                                            y_coords + y_coords[0:1], 
                                            top_z + top_z[0:1], 
                                            alpha=0.7, 
                                            color=plt.cm.terrain(z_height / np.max(height_map[valid_mask])))
                    
                    # 绘制侧面
                    for k in range(4):
                        next_k = (k + 1) % 4
                        # 侧面的四个顶点
                        side_x = [x_coords[k], x_coords[next_k], x_coords[next_k], x_coords[k]]
                        side_y = [y_coords[k], y_coords[next_k], y_coords[next_k], y_coords[k]]
                        side_z = [0, 0, z_height, z_height]
                        
                        self.ax.plot_trisurf(side_x, side_y, side_z, alpha=0.6, 
                                           color=plt.cm.terrain(z_height / np.max(height_map[valid_mask])))
